...
def main():
    ...
    start_url = 'https://buff.163.com/api/market/goods?game=csgo&page_num=1&search=%E8%83%B6%E5%9B%8A'
    max_page = get_max_page(start_url)
    max_page = int(max_page)
    #遍历所有页
    for index in range(0,max_page):
        url = 'https://buff.163.com/api/market/goods?game=csgo&page_num={}&search=%E8%83%B6%E5%9B%8A'.format(index+1)
...
def get_name_price(url,time):
    ...
    # print(result)
    result.to_csv('./胶囊/{}胶囊价格.csv'.format(time),index = False,header=0,mode='a+')
